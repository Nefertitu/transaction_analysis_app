...
def get_data_from_period(transactions: pd.DataFrame, date: str, time_range: str = "M") -> pd.DataFrame | Any:
    """
...
def get_event_page(transactions: pd.DataFrame) -> str:
    """
    Функция реализует функционал веб-страницы 'События', включающий предоставление
    следующих данных: расходы, поступления, курсы валют, стоимость акций из S&P 500
    :param transactions:
    ...
    """
    # Расходы
    expenses_category = transactions.loc[~transactions["Категория"].isin(["Зарплата", "Пополнения", "Бонусы", "Наличные", "Перевод с карты", "Переводы", "Перевод средств с брокерского счета"])]
    print(f"expenses_category:\n{expenses_category}")
    ...
    print(expenses_amount_category)
    ...
    # Переводы и наличные:
    cash_category = transactions.loc[result["Категория"].isin(["Наличные"])]
    cash_amount = round(abs(cash_category["Сумма операции"].sum()), 2)
    transfers_category = transactions.loc[transactions["Категория"].isin(["Перевод с карты", "Переводы", "Перевод средств с брокерского счета"])]
    transfers_amount = round(abs(transfers_category["Сумма операции"].sum()), 2)

    # Доходы
    income_category = transactions.loc[transactions["Категория"].isin(["Зарплата", "Пополнения", "Бонусы"])]
...
# print(result_1)
result = get_data_from_period(result_1, "2021-12-08", "W")
print(result)
data_json = get_event_page(result)
print(data_json)

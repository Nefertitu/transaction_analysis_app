...
def get_decorator(filename: str = "../report.log") -> Callable[[Callable], Callable]:
    """
...
def get_list_dict_transactions(transactions: pd.DataFrame | pd.Series) -> list[dict[Hashable, Any]]:
    """
    ...
    if isinstance(transactions, pd.DataFrame):
        ...
        result = [transactions.to_dict()]

    else:
        raise TypeError("transactions must be a pandas DataFrame or Series")

    return result


def get_to_json_investment_savings(month: str, transactions: list[dict[Hashable, Any]], limit: int | str) -> str:
    """
    ...
    result = {
        "Сумма инвестиционных накоплений": [f"{total_amount}"],
...
def get_choice_data(transactions: pd.DataFrame | pd.Series, date: str, time_range: str) -> pd.Series | Union[pd.DataFrame, None]:
    """
    Выбирает из предоставленного `DataFrame` данные за установленный период
    :param transactions:
    :param date:
    :param time_range:
    :return:

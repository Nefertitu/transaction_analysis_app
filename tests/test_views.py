...
def test_categorize_expenses_and_income(
    mock_get_exchange_rate, mock_get_stock_prices, transactions_random_data: pd.DataFrame, date: str, period: str
) -> None:
    """
    ...
    expected_result_rates = {"USD": {"currency": "USD", "rate": 91.75}, "EUR": {"currency": "EUR", "rate": 96.04}}
...
def test_get_event_page_success(mock_get_choice_data, mock_get_exchange_rate, mock_get_stock_prices, df_for_views):
    ...
    expected_result = {
        "expenses": {
            "total_amount": 422.37,
            "main": [[{"category": "Супермаркеты", "amount": 322.37}, {"category": "Остальное", "amount": 100.00}]],
            "transfers_and_cash": [
        ...
        "income": {
            "total_amount": 413.00,
            "main": [{"category": "Пополнения", "amount": 300.00}, {"category": "Бонусы", "amount": 113.00}],
        },
        "currency_rates": list(expected_result_rates.values()),
        "stock_prices": list(expected_result_stocks.values()),
    }

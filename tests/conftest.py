...
def data_for_test_pd_result() -> pd.DataFrame:
    ...
    return pd.DataFrame(sample_data).copy()
...
def data_for_test_pd_2() -> DataFrame:
    """Возвращает DataFrame с данными транзакций"""
    sample_data = {
        "Дата операции": ["2021-12-31", "2021-12-15"],
...
def data_for_test_pd_2_return() -> DataFrame:
    """Возвращает DataFrame с данными транзакций"""
    sample_data = {"Дата операции": ["2021-12-31", "2021-12-15"], "Сумма операции": [160.89, 64.00]}
...
def apilayer_responses() -> list[dict]:
    ...
    return [
        {"base": "USD", "date": "2025-02-07", "rates": {"RUB": 96.865166}, "success": True, "timestamp": 1738941317},
        {"base": "EUR", "date": "2025-02-07", "rates": {"RUB": 100.503777}, "success": True, "timestamp": 1738941545},
    ]
...
def financialmodel_responses() -> list[list[dict]]:
    ...
    return [
        [{"symbol": "AAPL", "price": 240.864, "volume": 36735534}],
        [{"symbol": "AMZN", "price": 229.6542, "volume": 25569805}],
        [{"symbol": "GOOGL", "price": 184.671, "volume": 59627739}],
        [{"symbol": "MSFT", "price": 409.42, "volume": 14782766}],
        [{"symbol": "TSLA", "price": 352.913, "volume": 55969352}],
    ]
...
def json_user_settings_1() -> dict:
    return {"url": "http://example.com/api/exchange-rate"}


@pytest.fixture
def json_user_settings_2() -> dict:
    return {"url": "https://site.financialmodelingprep.com/sample/"}
...
def apilayer_responses_views() -> dict:
    ...
    return {"USD": {"currency": "USD", "rate": 91.75}, "EUR": {"currency": "EUR", "rate": 96.04}}

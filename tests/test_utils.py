...
def test_configure_logging(tmp_path, sample_data_for_reports):
    ...
    logger = configure_logging(log_path=Path(filename))
    result = spending_by_workday(sample_data_for_reports, "2020-04-20", logger)
...
def test_configure_logging_2(tmp_path, sample_data_for_reports):
    ...
    filename = tmp_path / "mylog_reports.log"
    logger = configure_logging(log_path=Path(filename))
    result = spending_by_workday(sample_data_for_reports, logger=logger)
...
def test_get_read_excel(mock_read_excel, data_for_test_pd_result: DataFrame) -> None:
    """
    ...
    mock_read_excel.return_value = data_for_test_pd_result.copy()
    result = get_read_excel("test.xlsx")
    pd.testing.assert_frame_equal(result, data_for_test_pd_result.copy())
...
def test_get_formatted_date(data_for_test_pd_result, data_formatted_date_1):
    ...
    result = get_formatted_date(data_for_test_pd_result.copy())
...
def test_update_user_settings():
    ...
    expected_data = {"user_currencies": expected_currencies, "user_stocks": expected_stocks}
...
def test_get_choice_data(transactions_sample_data, date, time_range, expected_start_date, expected_end_date):
    ...
    filtered_df = get_choice_data(transactions_sample_data.copy(), date, time_range)
    assert len(filtered_df) > 0, "Фильтрация вернула пустой DataFrame"
    assert (
        filtered_df["Дата операции"].min() == expected_start_date
    ), f"Ожидаемая дата начала {expected_start_date}, полученная {filtered_df['Дата операции'].min()}"
    assert (
        filtered_df["Дата операции"].max() == expected_end_date
    ), f"Ожидаемая дата окончания {expected_end_date}, полученная {filtered_df['Дата операции'].max()}"
